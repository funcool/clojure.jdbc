...
your queries to return the primary keys of newly inserted records.
...
All good database libraries should come with connection pool support.

The Java ecosystem comes with various connection pool implementations for JDBC and clojure.jdbc
...
In order to use a connection pool, you should convert your plain dbspec into a datasource-dbspec
...
You can specify the transaction strategy to use in these two ways:
...
No. It is just an alternative implementation.
