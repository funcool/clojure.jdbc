...
NOTE: unlike clojure.java.jdbc, with _clojure.jdbc_ you can't use a plain dbspec hash-map as a
...
them: c3p0 and Apache Commons DBCP.

Install
^^^^^^^

Connection pool extensions lives as separate clojure packages and them should be installed
individually. All them works like other clojure packages: you only need put the dependency
on yout `project.clj` file.
...
`dbspec` now contains a `:datasource` key with a `javax.sql.DataSource` instance
instead of a plain dbspec with connection parameters. This dbspec should be used
like a plain dbspec for opening new connections.

Advanced configuration
^^^^^^^^^^^^^^^^^^^^^^

The two connection pool implementations comes with "good" defaults that should
work in majority standard environments, but obviously, it exposes set of options
for customize it:
...
Additionally to this options, some extensions offers other specific of each implementation, and you can view it
on source code of each extension. Also you can view the original documentation of them and apply additional options
after datasource dbspec is created.
...
- clojure.jdbc has a simpler implementation than clojure.java.jdbc. It has no more
  ...
  * clojure.java.jdbc has a lot boilerplate connection management around all functions
    ...
    +
    E.g.: functions like `create!` can receive plain a dbspec or a connection. If you are
    curious, take a look at the `with-db-connection` implementation of clojure.java.jdbc
    and compare it with `with-connection` of clojure.jdbc. You will get a get a good idea of the
    hidden unnecessary complexity found in clojure.java.jdbc. +
    +
...
Why does clojure.jdbc not include a DSL for working with SQL as clojure.java.jdbc 0.3 does?
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
Is this a fork of clojure.java.jdbc?
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
Additionaly, I want to give thanks to the `clojure.java.jdbc` developers for their good
